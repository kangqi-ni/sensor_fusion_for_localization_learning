...
#else 
    config_.q_parameterization_ptr = new PoseSO3Parametrization();
#endif
...
#else
    ceres::CostFunction *factor_edge = new LidarEdgeFactorAnalytic(
    source, 
            target_x, target_y, 
            ratio
    );
...
#else
    ceres::CostFunction *factor_plane = new LidarPlaneFactorAnalytic(
        source, 
        target_x, target_y, target_z, 
        ratio
    );
