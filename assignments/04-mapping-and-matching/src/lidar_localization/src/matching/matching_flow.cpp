...
enum RelocaliationMethod {
    GNSS,
    ScanContext
};

RelocaliationMethod relocalization_method = GNSS;
...
bool MatchingFlow::UpdateMatching() {
    if (!matching_ptr_->HasInited()) {
        ...
        if (relocalization_method == GNSS) {
            matching_ptr_->SetGNSSPose(current_gnss_data_.pose);
        }
        else if (relocalization_method == ScanContext) {
            matching_ptr_->SetScanContextPose(current_cloud_data_);
        }
        else {
            std::cerr << "Error: Unknown Relocalization Method\n"
                      << "Attempt to relocalization at the origin" << std::endl;
