 ...
 * @param  f_n, accel measurement in body frame
 * @return void
 */
...
void ErrorStateKalmanFilter::UpdateProcessEquation(
  ...
  Eigen::Vector3d f_b = C_nb.transpose()*(linear_acc_mid + g_); // Eigen::Vector3d f_n = linear_acc_mid + g_
...
void ErrorStateKalmanFilter::UpdateErrorEstimation(
  ...
  B.block<3,3>(kIndexErrorOri,kIndexNoiseGyro) = Eigen::Matrix3d::Identity() * T;
  B.block<3,3>(kIndexErrorAccel,kIndexNoiseBiasAccel) = Eigen::Matrix3d::Identity() * sqrt(T);
  B.block<3,3>(kIndexErrorGyro,kIndexNoiseBiasGyro) = Eigen::Matrix3d::Identity() * sqrt(T);
...
void ErrorStateKalmanFilter::EliminateError(void) {
  ...
  Eigen::Quaterniond rot = Eigen::Quaterniond(pose_.block<3,3>(0,0)).normalized();
  pose_.block<3,3>(0,0) = rot.toRotationMatrix();
